...
use indoc::indoc;
use mpatch::{apply_patch, parse_diffs, Patch};
use std::fs;
...
fn applying_benches(c: &mut Criterion) {
    ...
    let creation_setup = ApplyBenchSetup {
        ...
        .unwrap()
        .remove(0),
        initial_content: String::new(), // File doesn't exist initially
        target_filename: "new_file.txt".to_string(),
    };
    ...
    group.bench_function("file_creation", |b| {
        b.iter_with_setup(
            || {
                ...
                // Don't create the file; that's what we're testing
                (dir, creation_setup.patch.clone())
            },
            |(dir, patch)| {
    ...
    group.bench_function("exact_match_large_file", |b| {
        b.iter_with_setup(
            || {
                let dir = tempdir().unwrap();
                let file_path = dir.path().join(&exact_large_setup.target_filename);
                fs::write(file_path, &exact_large_setup.initial_content).unwrap();
                (dir, exact_large_setup.patch.clone())
            },
            |(dir, patch)| {
                apply_patch(black_box(&patch), black_box(dir.path()), false, 0.0).unwrap();
            },
